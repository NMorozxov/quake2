...
BUILD_QUAKE2=YES	# quake2 executable
...
ifneq ($(OSTYPE),SCO_SV)
BASE_CFLAGS=-Wall -pipe -Dstricmp=strcasecmp -I/usr/local/include
else
...
XCFLAGS=-I/usr/X11R6/include -I/usr/local/include
XLDFLAGS=-L/usr/X11R6/lib$(_LIB) -L/usr/local/lib -lX11 -lXext -lXxf86dga -lXxf86vm
AALDFLAGS=-lm -laa
...
ifneq ($(OSTYPE),SCO_SV)
SHLIBCFLAGS=-fPIC
SHLIBLDFLAGS=-shared -L/usr/local/lib
else
SHLIBCFLAGS=-KPIC
SHLIBLDFLAGS=-G -belf
endif
...
$(BUILDDIR)/quake2 : $(QUAKE2_OBJS) $(BUILDDIR)/ded/cd_null.o $(BUILDDIR)/client/snddma_null.o
	$(CC) $(CFLAGS) -o $@ $(QUAKE2_OBJS) $(BUILDDIR)/ded/cd_null.o $(BUILDDIR)/client/snddma_null.o $(LDFLAGS)
endif
